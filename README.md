...
检索要走本地嵌入模型，模型与**裁剪后的运行时**都**不在仓库里**（第三方制品，且体积远大于代码）。**先 `pnpm build`**——安装脚本按仓库里声明的期望身份自证，而那份声明的编译产物在 `dist/` 里；`dist/` 缺失时脚本会直接让你先构建，而不是抛一个找不到模块的堆栈。
...
**安装自证**：装完立刻按期望身份判定；不符就以非零状态退出并指名是哪一份不符，绝不报告成功。装到一半（断网、断电、复制途中出错）留下的内容也判不过，所以那半份永远不会被当成可用安装——不需要额外的回滚来保证这一点。

**`--check` 比内容、且区分两种情形**：文件不在报「缺失」，文件在但不是那一份报「不符」并同时给出期望与实际摘要，两种情况可以在同一次输出里分别标出。

**期望身份在仓库里**（`src/recall/model-expected.ts`）：哪些内容、各自必须是哪一份，随代码钉死，不随插件配置改变，也不来自安装记录。安装写下的 `MANIFEST.json` 只记录「这批字节从哪来」，不参与判定——改它不影响结论。第三方可用 `node scripts/verify-model-expected.mjs` 独立复核：运行时的 4 个受检文件与整包指纹从锁定版本的依赖就地重算，模型半从本机缓存或经代理重取比对；够不到的来源会明确报「未复核」，不会静默通过。

**同一台机器上的所有 profile 请保持同一插件版本**：模型目录与派生索引目录是共享的，而判定依据是每个 profile 自己那份声明。两个 profile 装的版本若声明不同——共享模型目录时，必有一个永远判定失败；共享索引目录时，两者会每轮把对方的索引判为需要重建（每轮一次全量重建）。要根治得按身份分目录，本期不做。

**不会静默下载**：运行时或模型缺失、或内容与声明不符时，检索在**调用点**抛明确错误（含本脚本的命令与体积），该次检索失败而会话继续可用；插件不会在后台拉 95 MB 模型，不会自动重装，也不会退化成另一种检索。把原因消除后，**紧接着的下一次检索就恢复**，不需要重启宿主。
