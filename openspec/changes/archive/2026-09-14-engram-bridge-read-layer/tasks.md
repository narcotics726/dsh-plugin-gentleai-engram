...
- [x] 7.1 **词法表逐行比对**（主要判据）：把实现指向冻结语料重建索引，与参考索引的 `docs` / `terms` / `postings` 逐行比对。
  - 结果：`docs` 346 / `terms` 28181 / `postings` 79157 **三表逐行相等**；参考的 12 个可比 meta 键全等（`algo_version` 为有意递增）；新增列 346 行与正本逐字节相等、不符 0；`source_hash` 与参考一致 `a3fa6aaa…`；向量 346 × 512。物证原在 `ops/readlayer-eng/accept/work/acceptance.json`，**已随 2026-09-15 的 `ops/` 清理删除**；读数见本行。
  - 验证：与参考实现建出的索引比对——除新增列外，`docs` / `terms` / `postings` 三张表逐行相等（参考侧规模为 346 文档 / 28181 词项 / 79157 posting，见 仓库外 spike `REPORT.md` 的 V5，该报告已于 2026-09-15 清理）；新增列（title/content/type/project/scope）与正本逐字节相等。
- [x] 7.2 逻辑指纹作为**辅助**判据：同一内容跨实现相等。
  ...
  - 验证：用 spike 的 `logical_digest.py`（已于 2026-09-15 清理；做法见内部笔记）比对两个独立构建，指纹相同；**不**把"等于旧值"当判据（扩列后必然改变）。
...
- [x] 7.5 内存判据（跨平台，唯一可能翻盘的量）。
  ...
  - 验证：在目标机器上跑 `mem_profile.ts`（spike 工具已于 2026-09-15 清理；做法见内部笔记），1 线程稳态 **≤ 600 MB**；超过则记录并回到 design 的 D4 重评线程档与路线。
