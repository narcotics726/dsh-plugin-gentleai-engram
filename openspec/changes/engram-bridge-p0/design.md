...
6. **会话身份 = dsh 会话标识，重复开始一律复用（同 id 幂等）。** 实测本构建只产生 `startup`（子会话亦然），`resume` 有生产者但 headless 无入口，`clear` / `compact` 无生产者——因此幂等要求按"同一标识重复开始不新建"写，并由单元测试覆盖；spec 不为 `clear` / `compact` 写场景（它们无法被触发）。**P0 不调用 `mem_session_end`**：1.20.0 虽允许重开，但文档语义不可逆，且 dsh 侧没有"会话结束"信号。
